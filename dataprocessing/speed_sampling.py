...
def arm_obstacle_distance(th_batch, chain, out_path_, vertices_obs, faces_obs):
    whole_dis = []
    end_dis = []
    batch_size = 50000
    for batch_id in range(math.floor(th_batch.shape[0]/batch_size)+1):
        ...
        distance, closest_faces, closest_points = igl.point_mesh_squared_distance(query_points, vertices_obs, faces_obs)
        ...
        del p, p_list, tg_batch, distance, query_points, whole_distance
...
def arm_append_list(X_list, Y_list, chain, out_path_, 
    ...
    while OutsideSize > 0:
        ...
        th_batch = scale*x0
        print(th_batch.shape)
        obs_distance0 = arm_obstacle_distance(th_batch, chain, out_path_, vertices_obs, faces_obs)
        ...
        th_batch = scale*x1
        obs_distance1 = arm_obstacle_distance(th_batch, chain, out_path_, vertices_obs, faces_obs)
...
def arm_rand_sample_bound_points(numsamples, dim, 
    ...
    X_list, Y_list = arm_append_list(X_list, Y_list, chain, out_path_, 
                                    v_obs, f_obs, numsamples, dim, offset, margin)
...
def point_obstacle_distance(query_points, vertices_obs, faces_obs):
    query_points = query_points.cpu().numpy()#.unsqueeze(dim=0)
...
def point_append_list(X_list,Y_list, 
                      vertices_obs, faces_obs, numsamples, dim, offset, margin):
    ...
    while OutsideSize > 0:
        ...
        obs_distance0 = point_obstacle_distance(x0, vertices_obs, faces_obs)
        where_d          =  (obs_distance0 > offset) & (obs_distance0 < margin)
        x0 = x0[where_d]
        x1 = x1[where_d]
        y0 = obs_distance0[where_d]

        obs_distance1 = point_obstacle_distance(x1, vertices_obs, faces_obs)
...
def point_rand_sample_bound_points(numsamples, dim, 
    ...
    X_list, Y_list = point_append_list(X_list,Y_list,  v_obs, f_obs, numsamples, dim, offset, margin)
